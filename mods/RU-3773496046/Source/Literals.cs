...
namespace Igor.RU_3773496046
{
    public sealed class LiteralTranslations : Mod
    {
        private static readonly Dictionary<string,Dictionary<string,string>> Methods =
            ...
            { "NiceResearchTab.CurrentResearchWidget:Draw", new Dictionary<string,string> {
                { "Remaining time: N/A", "Оставшееся время: неизвестно" }
            } },
            { "NiceResearchTab.DebugOptionsWidget:.ctor", new Dictionary<string,string> {
                { "Add 25% gravdata", "Добавить 25% гравиданных" },
                { "Apply techprint", "Применить техпринт" },
                { "Finish now", "Завершить сейчас" }
            } },
            { "NiceResearchTab.MainTabWindow_ResearchTree:PreOpen", new Dictionary<string,string> {
                { "DEV", "Разработчик" }
            } },
            { "NiceResearchTab.Settings:DrawPacker", new Dictionary<string,string> {
                { "{0} ms", "{0} мс" }
            } },
            { "NiceResearchTab.Settings:DrawPerformance", new Dictionary<string,string> {
        ...
        private static readonly Dictionary<string,string[]> Parameters = new Dictionary<string,string[]> {
            { "NiceResearchTab.CurrentResearchWidget:Draw", new string[] {  } },
            { "NiceResearchTab.DebugOptionsWidget:.ctor", new string[] { "System.String", "System.Single" } },
            { "NiceResearchTab.MainTabWindow_ResearchTree:PreOpen", new string[] {  } },
            { "NiceResearchTab.Settings:DrawPacker", new string[] { "UnityEngine.Rect" } },
            { "NiceResearchTab.Settings:DrawPerformance", new string[] { "UnityEngine.Rect" } }
        };
        public LiteralTranslations(ModContentPack content) : base(content)
        {
            var harmony = new Harmony("igor.ru.3773496046.literals");
