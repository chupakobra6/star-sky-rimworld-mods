# Character Editor — русский перевод [Star Sky Mods]

[![Character Editor — русский перевод](https://raw.githubusercontent.com/chupakobra6/star-sky-assets/main/assets/character-editor-translation.png)](https://steamcommunity.com/sharedfiles/filedetails/?id=3807830501)

[Мастерская](https://steamcommunity.com/sharedfiles/filedetails/?id=3807830501) · [Все категории](../../README.md) · [Источники и уведомления](NOTICES.md)

Русский перевод Character Editor для RimWorld 1.6. Оригинальный мод устанавливается отдельно.

Character Editor позволяет редактировать персонажей до высадки и во время игры. Этот пакет переводит окна редактора, настройки и команды на русский язык.

## Что переведено

<!-- rimmods:readme features begin -->
Переведены игровые тексты оригинального мода.

- Поля внешности, имени, навыков и черт персонажа.
- Команды редактирования снаряжения, здоровья и отношений.
- Настройки редактора и сообщения об изменениях персонажей.
<!-- rimmods:readme features end -->

## Установка

<!-- rimmods:readme installation begin -->
1. Установите [Harmony](https://steamcommunity.com/sharedfiles/filedetails/?id=2009463077) и [Character Editor](https://steamcommunity.com/sharedfiles/filedetails/?id=1874644848).
2. Включите моды в порядке: Harmony → Character Editor → этот перевод.
3. Выберите русский язык в настройках RimWorld.
<!-- rimmods:readme installation end -->

Откройте Character Editor при создании колонии или во время игры, выберите персонажа и нужный раздел редактирования.

## Обратная связь

<!-- rimmods:readme feedback begin -->
Заметили ошибку или неточность? Напишите в комментариях на странице мода или на Boosty.
<!-- rimmods:readme feedback end -->

<!-- rimmods:readme credit begin -->
Автор оригинального мода: void.
<!-- rimmods:readme credit end -->

## Лицензии

Собственные русские тексты выпускаются под CC BY-SA 4.0. Собственный код интеграции — MIT. Полный текст основной лицензии и сведения об источниках находятся в [LICENSE](LICENSE) и [NOTICES.md](NOTICES.md).

## Уведомление Ludeon

<!-- rimmods:readme ludeon begin -->
Portions of the materials used to create this content/mod are trademarks and/or copyrighted works of Ludeon Studios Inc. All rights reserved by Ludeon. This content/mod is not official and is not endorsed by Ludeon.
<!-- rimmods:readme ludeon end -->
