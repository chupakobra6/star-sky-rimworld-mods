...
namespace Igor.RU_3033901359
{
    public sealed class LiteralTranslations : Mod
    {
        private static readonly Dictionary<string,Dictionary<string,string>> Methods =
            ...
            { "AdaptiveStorage.AdaptiveStorageFrameworkMod:SettingsCategory", new Dictionary<string,string> {
                { "Adaptive Storage Framework", "Adaptive Storage Framework — адаптивные хранилища" }
            } },
            { "AdaptiveStorage.Listing_TreeDefs:ValueEditWidgetRight", new Dictionary<string,string> {
                { "uneditable value type", "значение этого типа нельзя редактировать" }
            } },
            { "AdaptiveStorage.TreeNode_Editor:get_ExtraInfoText", new Dictionary<string,string> {
                { "elements", "эл." },
                { "element", "эл." }
            } }
            };
        ...
        {
            var harmony = new Harmony("igor.ru.3033901359.literals");
