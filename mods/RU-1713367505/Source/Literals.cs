...
namespace Igor.RU_1713367505
{
    public sealed class LiteralTranslations : Mod
    {
        private static readonly Dictionary<string,Dictionary<string,string>> Methods =
            ...
            { "aRandomKiwi.ARS.RimSaves:SettingsCategory", new Dictionary<string,string> {
                { "RimSaves", "RimSaves — сохранения" }
            } },
            { "aRandomKiwi.ARS.Utils:FormatSize", new Dictionary<string,string> {
                { "B", " Б" },
                { "KB", " КБ" },
                { "MB", " МБ" },
                { "GB", " ГБ" },
        ...
        {
            var harmony = new Harmony("igor.ru.1713367505.literals");
