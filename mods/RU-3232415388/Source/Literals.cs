...
namespace Igor.RU_3232415388
{
    public sealed class LiteralTranslations : Mod
    {
        private static readonly Dictionary<string,Dictionary<string,string>> Methods =
            ...
            { "SimpleCameraSetting.CameraDriverOnGUIPatch:Prefix", new Dictionary<string,string> {
                { "Current Zoom : {0:F2}          ", "Текущий масштаб: {0:F2}" },
                { "Camera Following ", "Слежение камеры: " },
                { "Off", "выключено" },
                { "On", "включено" }
            } },
            { "SimpleCameraSetting.CameraMapConfigUpdatePatch:FollowPostfix", new Dictionary<string,string> {
                { "Camera Following Off", "Слежение камеры выключено" }
            } },
            { "BetterKeybinding.KeyBind:get_Label", new Dictionary<string,string> {
                { "Unknown", "Неизвестная клавиша" }
            } },
            { "BetterKeybinding.KeyBind:get_LabelShort", new Dictionary<string,string> {
                { "Unknown", "Неизвестная клавиша" }
            } },
            { "BetterKeybinding.KeyBind:ButtonLabel", new Dictionary<string,string> {
                { "LMB", "ЛКМ" },
                { "RMB", "ПКМ" },
                { "MMB", "СКМ" },
                { "Button {0}", "Кнопка мыши {0}" }
            } },
            { "BetterKeybinding.KeyBind:ScrollLabel", new Dictionary<string,string> {
                { "ScrollDown", "Прокрутка вниз" },
                { "ScrollUp", "Прокрутка вверх" },
                { "ScrollLeft", "Прокрутка влево" },
                { "ScrollRight", "Прокрутка вправо" },
                { "ScrollUnknown", "Неизвестное направление прокрутки" }
            } },
            { "SimpleCameraSetting.SimpleCameraModSetting:DoSettingsWindowContents", new Dictionary<string,string> {
                { "ZoomRange", "chupakobra6_3232415388_ZoomRange" }
            } }
            };
        ...
        {
            var harmony = new Harmony("igor.ru.3232415388.literals");
