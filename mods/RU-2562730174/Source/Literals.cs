...
namespace Igor.RU_2562730174
{
    public sealed class LiteralTranslations : Mod
    {
        private static readonly Dictionary<string,Dictionary<string,string>> Methods =
            ...
            { "VisibleRaidPoints.TextGenerator:GetPsychicRitualSiegeThreatDesc", new Dictionary<string,string> {
                { " ({0} points)", " (очков: {0})" }
            } },
            { "VisibleRaidPoints.TextGenerator:GetThreatPointsBreakdownText", new Dictionary<string,string> {
                { "Debug info:", "Сведения для отладки:" },
                { "\nCalling method: ", "\nВызывающий метод: " }
            } },
            { "VisibleRaidPoints.VisibleRaidPointsMod:SettingsCategory", new Dictionary<string,string> {
                { "Visible Raid Points", "Visible Raid Points — очки угрозы" }
            } }
            };
        ...
        {
            var harmony = new Harmony("igor.ru.2562730174.literals");
